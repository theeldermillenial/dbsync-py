...
class TestFoundationModelsIntegration(BaseIntegrationTest):
    ...
    def test_chain_meta_table_exists(self, dbsync_session):
        ...
        # Verify table exists
        assert self.verify_table_exists(dbsync_session, "meta"), "meta table should exist"
    ...
    def test_extra_migrations_table_exists(self, dbsync_session):
        ...
        # Verify table exists
        assert self.verify_table_exists(dbsync_session, "extra_migrations"), "extra_migrations table should exist"
    ...
    def test_event_info_table_exists(self, dbsync_session):
        ...
        # Verify table exists
        assert self.verify_table_exists(dbsync_session, "event_info"), "event_info table should exist"
    ...
    def test_chain_meta_database_schema_compliance(self, dbsync_session):
        ...
        assert not db_columns['network_name']['nullable'], "network_name should be NOT NULL"
        ...
        assert 'timestamp' in str(db_columns['start_time']['type']).lower(), "start_time should be timestamp"
        assert 'varchar' in str(db_columns['network_name']['type']).lower() or 'character varying' in str(db_columns['network_name']['type']).lower(), "network_name should be varchar"
        assert 'varchar' in str(db_columns['version']['type']).lower() or 'character varying' in str(db_columns['version']['type']).lower(), "version should be varchar"
    ...
    def test_extra_migrations_database_schema_compliance(self, dbsync_session):
        ...
        # Check data types
        assert 'bigint' in str(db_columns['id']['type']).lower(), "id should be bigint"
        assert 'varchar' in str(db_columns['token']['type']).lower() or 'character varying' in str(db_columns['token']['type']).lower(), "token should be varchar"
        assert 'varchar' in str(db_columns['description']['type']).lower() or 'character varying' in str(db_columns['description']['type']).lower(), "description should be varchar"
    ...
    def test_event_info_database_schema_compliance(self, dbsync_session):
        ...
        for field in expected_db_fields:
            assert field in actual_db_fields, f"Database missing expected field: {field}"
        ...
        # Check data types
        assert 'bigint' in str(db_columns['id']['type']).lower(), "id should be bigint"
        assert 'bigint' in str(db_columns['tx_id']['type']).lower(), "tx_id should be bigint"
        ...
        assert any(t in epoch_type_str for t in ['integer', 'word31type', 'domain']), f"epoch should be integer, word31type, or domain, got: {epoch_type_str}"
        assert 'varchar' in str(db_columns['type']['type']).lower() or 'character varying' in str(db_columns['type']['type']).lower(), "type should be varchar"
        assert 'varchar' in str(db_columns['explanation']['type']).lower() or 'character varying' in str(db_columns['explanation']['type']).lower(), "explanation should be varchar"
    ...
    def test_chain_meta_model_field_mapping_issues(self, dbsync_session):
        ...
        for col_name in expected_columns:
            assert col_name in actual_columns, f"Model missing database column mapping: {col_name}"
    ...
    def test_event_info_model_field_mismatch(self, dbsync_session):
        ...
        missing_in_model = db_fields - model_fields - {'id'}  # Exclude id as it's mapped to id_
        extra_in_model = model_fields - db_fields - {'id_'}   # Exclude id_ as it maps to id
        
        assert not missing_in_model, f"Model missing database fields: {missing_in_model}"
        assert not extra_in_model, f"Model has extra fields not in database: {extra_in_model}"
    ...
    def test_event_info_database_insert_fails_completely(self, dbsync_session):
        ...
        try:
            ...
            pytest.fail("EventInfo insert should have failed - model schema is completely wrong")
    ...
    def test_model_table_creation_matches_database(self, dbsync_session):
        ...
        for model_class, table_name in models_to_test:
            ...
            assert not missing_in_model, f"{model_class.__name__} missing columns: {missing_in_model}"
            assert not extra_in_model, f"{model_class.__name__} has extra columns: {extra_in_model}"
    ...
    def test_meta_table_not_null_constraints(self, dbsync_session):
        ...
        for test_data in test_cases:
            try:
                ...
                pytest.fail(f"Database should have rejected incomplete data: {test_data}")
